      ...
      <main class="prose p-4">
        <article>
          ...
          <div class="inline-block w-auto text-xs text-gray-500 pt-1 pb-1 pl-3 mr-2 rounded ">
            Updated <time datetime="{{ date | machineDate }}">{{ date | readableDate }}</time>
          </div>
          ...
          <p>
            ${entry
              ...
              .map(
                (tag) =>
                  html`
                    <a class="text-xs mr-2 px-3 py-2 text-gray-600 bg-gray-200 no-underline rounded" href="#" rel="tag"
                      >${tag}</a
                    >
                  `,
              )}
